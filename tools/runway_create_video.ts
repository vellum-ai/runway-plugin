...
const tool: ToolDefinition = {
  ...
  description: "Start an explicitly user-approved Runway Gen-4.5 generation; requires the exact approval ID from runway_prepare_video, spends Runway credits, and returns a task ID.",
  ...
  input_schema: {
    ...
    properties: {
      ...
      approval_id: { type: "string", minLength: 1, description: "Approval ID returned by runway_prepare_video for these exact prompt and settings." },
      user_approved: { type: "boolean", description: "Must be true only after the user explicitly approves the exact prepared prompt and settings." }
    },
    required: ["prompt", "approval_id", "user_approved"]
  },
